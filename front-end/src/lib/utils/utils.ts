...
export const removeAllEmpty = (form: any) => {
	const newForm = {} as any;
	Object.keys(form).forEach(key => {
		if (!isEmpty(getField(form, key))) {
			newForm[key] = getField(form, key);
		}
	});
	return newForm;
...
export function searchOnObject(obj: any, listOfFields: string[], searchValue: string) {
	if (!obj || !listOfFields || listOfFields.length < 0 || !searchValue) {
		return false;
	}
	let isInclude = false;
	listOfFields.forEach(v => {
		if (!!obj[v]) {
			isInclude = isInclude || searchOnValue(`${ obj[v] }`.toLowerCase(), searchValue.toLowerCase());
		}
	});
	return isInclude;
}