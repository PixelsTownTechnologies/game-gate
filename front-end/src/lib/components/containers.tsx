	...
	return (
		<FlexBox dir={ props.dir } flexDirection={ 'column' }>
			<FlexBox warp dir={ props.dir } justifyContent={ 'space-between' } alignItems={ 'center' }>
				<FlexCenter className={ 'px-title-header' } dir={ props.dir }>
					<If flag={ props.icon }>
						<Icon size={ 'big' } name={ props.icon as any }/>
					</If>
					<Header className={ 'px-non-margin global-font' }
					        as={ 'h1' }>{ pxIfSelf(props.title, 'No Title') as string }</Header>
				</FlexCenter>
				<If flag={ props.subTitleChildren }>
					<div dir={ props.dir }>
						{ props.subTitleChildren }
					</div>
				</If>
			</FlexBox>
			{
