...
import { useLoader, UseStateLoader } from "../../../../lib/hooks/generic";
...
import { costFormat, isEmpty, isNull, isTrue } from "../../../../lib/utils/utils";
...
export function OrderWidgetView({orderView, loader, hideKeySection, hideReviewSection}: {
	orderView: OrderDTO,
	loader?: UseStateLoader,
	hideKeySection?: boolean,
	hideReviewSection?: boolean,
}) {
	const [ stateOrder, setOrder ] = useState(orderView);
	const order = !isTrue(hideReviewSection) ? stateOrder : orderView;
	...
	return (
		<FlexBox dir={ dir } flexDirection={ 'column' } pxIf={ !!order }>
				...
				<FlexBox warp padding={ 20 } className={ 'order-data' }>
					<Form.Field>
						...
						<Header className={ 'global-f px-non-margin' }>
							{ order.id.toPrecision(8).split('.').reverse().join('') }
						</Header>
					</Form.Field>
					<Form.Field>
						<label
					...
					<If flag={ ( !!order.accessory && !!order.accessory.id && !!order.ship_location ) }>
						<Form.Field>
							<label
								className={ 'global-f order-data-label' }>{ words.entities.order.ship_location }</label>
							<Header
								className={ 'global-f px-non-margin' }>{ order.ship_location }</Header>
						</Form.Field>
					</If>
			...
			<Divider hidden/>
			{
				order?.accessory?.id ? (
					<FlexBox
						className={ 'order-confirm-container gray-bg' }
					...
					>
						<div>
							<Image width={ 70 } src={ order.accessory.logo }/>
						</div>
						<FlexBox
							justifyContent={ isMobile ? 'center' : undefined }
							alignItems={ isMobile ? 'center' : undefined }
							warp flexDirection={ 'column' } padding={ 30 }
						>
							...
							<Header className={ 'state-info' } as={ 'h2' }>
								{ words.viewer.itemID }:
								<Link
									to={ URL_ROUTES.ACCESSORY_VIEWER + `/${ order.accessory.id }` }>{ order.accessory.id.toPrecision(8).split('.').reverse().join('') }</Link>
							</Header>
						</FlexBox>
					</FlexBox>
				) : null
			}
			{
				order?.game_card?.id ? (
					...
					>
						<div>
							<ImageCard logo={ order.game_card.game?.logo } title={ order.game_card.game?.card_name }/>
						</div>
						<FlexBox
							justifyContent={ isMobile ? 'center' : undefined }
							alignItems={ isMobile ? 'center' : undefined }
						...
						>
							...
							<Header dir={ dir } className={ 'state-info' } as={ 'h2' }>
								...
								<Link
									...
									{ order?.game_card.id.toPrecision(8).split('.').reverse().join('') }
								</Link>
							</Header>
						</FlexBox>
					</FlexBox>
							...
							onClick={ () => {
								if (!( isReviewDisabled || loader?.isLoading || reviewForm.review_star < 1 )) {
									...
									new EntityService(userOrderService).updateEntity(order.id, {
										...
										if (r) {
											setOrder({...order, ...r});
											new EntityService(userOrderService).reload().then((data) => {
												loader?.disabled?.();
											});
										}
									});
			...
			<Divider hidden/>
			{
				order.game_card && !isEmpty(order.order_keys) && !isTrue(hideKeySection) ? (
					<FlexBox>
						<Divider hidden/>
						<Divider hidden/>
...
export function SingleOrderHistory(props: BaseRouteComponentProps) {
	...
	return (
		<Wrapper loading={ loader.isLoading } icon={ 'clipboard' }
			...
			{
				order ? <OrderWidgetView loader={ loader } orderView={ order }/> : null
			}
		</Wrapper>
	);
}
