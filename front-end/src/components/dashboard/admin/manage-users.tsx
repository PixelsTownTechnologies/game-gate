...
import { getDefaultValidMsg, removeAllEmpty } from "../../../lib/utils/utils";
...
class ManageUsers extends EntityWrapper<UserBaseDTO, ManageUsersProps, ManageUsersState> {
	...
	getFormFields(): DFormField[][] {
		const words: LanguageSystemWords = this.state.word as LanguageSystemWords;
		return [
			[
				{
					fieldName: 'email',
	...
	async handleSaveAction(form: UserBaseDTO | { groups: string[] }): Promise<DialogFormActionResult> {
		...
		return super.handleSaveAction(this.state.action === 'add' ? removeAllEmpty(form) : form);
	}
